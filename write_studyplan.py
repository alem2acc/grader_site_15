...
STUDY_PLAN = r'''import { useState, useEffect, useCallback } from "react";
                  ...
                  <div key={stage.level} style={{ display: "flex", alignItems: "center", flex: 1 }}>
                    <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 0 }}>
                      ...
                      <div style={{ position: "relative" }}>
                        {isTarget && !isCurrent && (
                          <div style={{ position: "absolute", top: -22, left: "50%", transform: "translateX(-50%)", fontSize: 9, fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", color: "#0a0a0a", whiteSpace: "nowrap", background: "#0a0a0a", color: "#fff", padding: "2px 8px", borderRadius: 100 }}>
                            Target
                          </div>
                        )}
                        <div style={{
...
path = os.path.join(BASE, "StudyPlan.tsx")
with open(path, "w", encoding="utf-8") as f:
    f.write(STUDY_PLAN)
print(f"OK  StudyPlan.tsx  ({len(STUDY_PLAN):,} chars)")
